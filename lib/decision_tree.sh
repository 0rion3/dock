# IMPORTANT!!!
# This is THE DECISION TREE implementation as it is described in the corresponding
# section of the README file. As this is the most important part, DO NOT deviate
# from the behavior describe in README or update README immediately if any changes
# were made. It may not seem like much, as the description is probably much more
# elaborate than the implementation. But it has to be handled (fixed, improved, changed)
# with special care and attention to details.
#
...
if [[ -n $pos_arg1 ]] && [[ -n $pos_arg2 ]]; then
  ...
  # If we don't exit by now, then image and container both exist or only image exists.
  # Fulfils THE DECISION TREE case 1.1 and 1.2 as ./container_info
  # provides us with the $create_new_container variable which instructs
  # the ./start_container script to create a new container from the image first.
  #
  ...
  if [[ -n $container_name ]]; then
    ...
    if [[ -n $image_name ]]; then
      if [[ "$container_image_name" == "$full_image_name" ]]; then
        ...
        # THE DECISION TREE case 2.2.b fullfiled:
        # a CONTAINER resembling current directory name doesn't exist, so it's a fine assumption
        # user wants to create a new one based on the image which name they provided in
        # that single positional argument.
        decision_tree_result="2.2.b"
        container_name="$project_name"
      else
        # THE DECISION TREE case 2.2.c fulfilled: IMAGE found, but an existing
        # CONTAINER whith the name associated with the current directory is based on
        # a different imamge. We inform the user that two containers with the same
        # name are not allowed and they must specify container name explicitly in
        ...
        >&2 printf "$(ind 4;color gray)If you've made a mistake thinking the container did't exist and you actually meant\n"
      ...
      # THE DECISION TREE case 2.2.c fulfilled: no matching container, nor image is found
      # so we'll have to exit with an error.
...
# THE DECISION TREE SECTION (3) - no positional arguments, everything id derived from
# the current directory path - $PWD variable supplied by
# the ./directory.sh script.
else
  project_info="$($DOCK_PATH/bin/project_info $PWD)"
    echo "$project_info" | assign_vars_from_out
  container_info="$($DOCK_PATH/bin/container_info $project_name)"
  ...
  if [[ -n "$container_name" ]]; then
    ...
    # being derived from the current path. We now call ./image_info as we did in 2.2.a,
    # HOWEVER, the DIFFERENCE HERE is that we use the image name that we got from the container info,
    # And container name use to get that info was, in turn, was implicitly derived from the
    # current directory name and - AND NOT provided by the user.
    ...
    if [[ -n "$image_info" ]]; then
      ...
      # new contrainer with its name derived from the current directory
